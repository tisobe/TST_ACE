...
#
#
# modified Feb 13, 2015   by t. isobe (tisobe@cfa.harvard.edu)
# 

$SPACE_ftp        = 'ftp://ftp.swpc.noaa.gov/pub/lists/ace/';
$RACE_file        = '/data/mta4/proj/rac/ops/ACE/ace.archive';
$ephem_file       = '/data/mta4/proj/rac/ops/ephem/gephem.dat';
$fluence_file     = '/data/mta4/proj/rac/ops/ACE/fluace.dat';
$fluence_file_bak = '/data/mta4/proj/rac/ops/ACE/fluace.dat.good';
$fluence_archive  = '/data/mta4/proj/rac/ops/ACE/fluace.arc';
$falert_file      = '/data/mta4/proj/rac/ops/ACE/falert.dat';

@file_list = find_file_name();

foreach $ent (@file_list){
    $SPACE_file = "$SPACE_ftp"."$ent";
    run_main();
}
...
sub find_file_name{
    $checked_list = '/data/mta4/proj/rac/ops/ACE/ace_epam_ftp_list';
    
    open(FH, $checked_list);
    @old = ();
    while(<FH>){
        chomp $_;
        $last_entry = $_;
    ...
    $ftp   = `/usr/bin/lynx -source ftp://ftp.swpc.noaa.gov/pub/lists/ace/`;
    @data  = split(/\n+/, $ftp);
    @alist = ();
    @list  = ();
    $chk   = 0;
    foreach $ent (@data){
        if($ent =~/_ace_epam_5m.txt/){
            @atemp = split(/ace\//, $ent);
            $file  = $atemp[1];
            $file  =~ s/\"//g;
            if($file =~ /\n/ || $file eq ''){
